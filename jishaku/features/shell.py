...
class ShellFeature(Feature):
    ...
    async def jsk_git(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore
        ...
        if ctx.author.id in owners: 
         ...
         await ctx.invoke(self.jsk_shell, argument=Codeblock(argument.language, "git " + argument.content))  # type: ignore
        else:
            return
    ...
    async def jsk_pip(self, ctx: commands.Context, *, argument: codeblock_converter):  # type: ignore
        ...
        if ctx.author.id in owners: 
         ...
         await ctx.invoke(self.jsk_shell, argument=Codeblock(argument.language, f"{executable} {argument.content}"))  # type: ignore
        else:
            return
